...
const prepare = (ids, details, metadata, counter) => {
  ...
  const todayStart = DateTime.local().startOf("day").toISO();
  ...
  return {
    ...
    year_daily: group(
      sorted.filter(
        (d) =>
          DateTime.fromISO(d.time).startOf("day") <
          DateTime.local().startOf("day")
      ),
      dayFmt,
      oneYear
    ), // Convertir les dates en heures locales avant de comparer
    year_hourly: sorted.filter((d) => d.time >= oneYear)
  };
};
